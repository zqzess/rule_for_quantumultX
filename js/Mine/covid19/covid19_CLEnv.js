...
$.get(option, function (error, response, data) {
    ...
    if(jsonObj.Result.length !== 0)
    {
        ...
        $.log('\nLog:\n' + notifyContent)
        $.isSurge()?body = {title: '😷' + city, content: notifyContent, icon: 'cross.circle', 'icon-color': '#ff0000'} : body = {title: '😷' + city, content: notifyContent, icon: 'cross.circle', backgroundColor: '#ff0000'}
    } else {
        $.log('\nLog:\n省份或城市错误，没有获取到信息')
        notifyContent = '省份或城市错误，没有获取到信息!'
        if ($.isNotify==='true')
            $.msg('📢COVID-19', '❌错误', notifyContent)
        $.isSurge()?body = {title: '😷' + city, content: '❌错误\n' + notifyContent, icon: 'cross.circle', 'icon-color': '#ff0000'} : body = {title: '😷' + city, content: '❌错误\n' + notifyContent, icon: 'cross.circle', backgroundColor: '#ff0000'}
    }
